...
class MakePOT {
	...
	private function xgettext($project, $dir, $output_file, $placeholders = array(), $excludes = array(), $includes = array(), $multiDir = false) {
		...
		if (false === $multiDir) {
			...
			$originals = $this->extractor->extract_from_directories( $dir, $excludes, $includes );
		}
	 ...
	 * 
	 */
	public function wp_combined($pluginDir, $themeDir, $output, $slug = null)
	{
		$defaults = array(
			'excludes' => array(),
			'includes' => array(),
		...
		// guess plugin slug
		if (is_null($slug)) {
			$slug = $this->guess_plugin_slug($pluginDir);
		}

		$plugins_dir = @opendir( $pluginDir );
		$plugin_files = array();
		if ( $plugins_dir ) {
			while ( ( $file = readdir( $plugins_dir ) ) !== false ) {
				if ( '.' === substr( $file, 0, 1 ) ) {
		...
		$main_file = '';
		foreach ( $plugin_files as $plugin_file ) {
			if ( ! is_readable( "$pluginDir/$plugin_file" ) ) {
				continue;
			}

			$source = $this->get_first_lines( "$pluginDir/$plugin_file", $this->max_header_lines );

			// Stop when we find a file with a plugin name header in it.
			if ( $this->get_addon_header( 'Plugin Name', $source ) != false ) {
				$main_file = "$pluginDir/$plugin_file";
				break;
			}
		}
		...
		// last argument true to indicate the extractor needs to treat multiple directories
		$res = $this->xgettext( 'wp-plugin', array($pluginDir, $themeDir), $output, $placeholders, $args['excludes'], $args['includes'], true);
